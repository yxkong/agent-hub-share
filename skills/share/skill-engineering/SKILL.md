...
description: 创建、提炼、审查、重构和基于真实任务优化 AI Skill（skill creation, repository extraction, trigger eval, rollout optimization）。适用于从陌生仓库的架构/业务流/数据流抽项目技能、修正 description/trigger/SOP/references、用 baseline + held-out 减少返工；新建前必须先走 skill-discovery，且不负责真实业务代码。
---
...
| 任务类型 | 什么时候选它 | 先读什么 |
|------|------|------|
| `create` | skill-discovery 已证明无 exact/partial 可复用候选，需要从零创建 | `references/workflow/creation_workflow.md` |
| `extract` | 从存量代码的架构、业务流、数据流与主链提炼项目 skill | `references/workflow/legacy_project_extraction.md` + `references/workflow/codebase_evidence_views.md` |
...
| `optimize` | 用真实任务减少触发、路由、执行或证据返工 | `references/optimization/workflow.md` + `references/optimization/evidence_contract.md` |

规则：一次只走一条主路由；**先读路由前必读，再读对应 P0**；如果问题是"研发任务怎么推进"，立即转 `delivery-workflow`。

## 作用边界

**覆盖**：创建 / 提炼 / 审查 / 重构技能；从代码库证据提炼项目技能；优化 description / trigger / SOP / references / eval；执行 baseline → reflection → bounded edit → held-out 闭环；识别坏味道并沉淀规则。
...
- **创建前发现门**：`create` 必须提供 `skill-discovery` 候选表，并证明没有 exact/partial 可复用项；否则只能复用、适配或评审，不得新建同义 skill。
- **代码库证据门**：`extract` 必须覆盖逻辑架构、模块入口、业务主链、数据流/状态和至少一条失败链；目录树或抽象分层不能单独证明项目事实。
- **优化证据门**：`optimize` 没有 baseline 只能静态 review，没有 held-out 不得宣称通用改进；每轮只允许 `add / delete / replace` 一种主要编辑。
...
- **shared 通用性与闭包门**：`skills/share` 只承载跨项目可迁移动作规则；声明 `skill-package.json` 的技能必须只依赖自身资产或 manifest 中的 required skill，并通过隔离 smoke；运行环境装配成功不能替代包闭包证据，细则见 `references/layout/share_portability_contract.md`。
...
- `references/workflow/legacy_project_extraction.md` + `references/workflow/codebase_evidence_views.md` → `extract`
...
- `references/optimization/workflow.md` + `references/optimization/evidence_contract.md` → `optimize`
...
- create / extract / review / refine-trigger / optimize 结束前已检查坏味道并按需沉淀。
- create 已有 discovery 无重复候选证据；extract 已有代码库证据视图；optimize 已有 baseline 与 held-out，或明确标 `unknown`。
...
- **should-trigger**：新建/提炼/审查 skill、从陌生仓库抽项目技能、优化 description / trigger / SOP、用 rollout 验证行为有效性、沉淀坏味道规则
...
| 领域技能 | 落地业务代码 |
| `skill-scorecard` | 需要独立评分、门禁结论和优化前后对比时 |
