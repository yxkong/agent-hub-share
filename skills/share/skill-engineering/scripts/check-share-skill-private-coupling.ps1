...
function Test-SharePrivateCoupling {
    ...
    $localHubPathRegex = ('D:' + '\\' + '\\' + 'ai' + '\\' + '\\') + '|' + ('D:' + '/' + 'ai' + '/')
    $privateHubRegex = ('ai-hub' + '-private') + '|' + ('yxkong/ai-hub' + '-private')
    $privateProjectRegex = ('platform-jdk' + '17') + '|' + ('platform-' + 'admin') + '|' + ('platform-' + 'review') + '|' + ('platform-backend' + '-dev') + '|' + ('platform-' + 'core')
    $privateModuleRegex = ('admin-system' + '-base(?:-frontend)?')
    $legacyDocsRegex = 'PLATFORM' + '_DOCS_GOVERNANCE'
    $privateMediaRegex = ('wechat-article' + '-workflow') + '|' + ('skills/' + 'media/')
