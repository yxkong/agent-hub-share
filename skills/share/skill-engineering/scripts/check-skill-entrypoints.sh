...
check_front_matter() {
  file=$1
  first_line=$(sed -n '1p' "$file" | tr -d '\015')
  ...
  closing_line=$(awk 'NR > 1 {sub(/\015$/, "")} NR > 1 && $0 == "---" { print NR; exit }' "$file")
  [ -n "$closing_line" ] || {
    printf 'SKILL_ENTRYPOINT_VIOLATION=%s reason=%s\n' "$file" 'Missing closing front matter delimiter'
    COUNT=$((COUNT + 1))
    return
  }
  sed -n "2,$((closing_line - 1))p" "$file" | tr -d '\015' | grep -Eq '^name:[[:space:]]*[^[:space:]]+' || {
    printf 'SKILL_ENTRYPOINT_VIOLATION=%s reason=%s\n' "$file" 'Missing name in front matter'
    COUNT=$((COUNT + 1))
    return
  }
  sed -n "2,$((closing_line - 1))p" "$file" | tr -d '\015' | grep -Eq '^description:[[:space:]]*[^[:space:]]+' || {
