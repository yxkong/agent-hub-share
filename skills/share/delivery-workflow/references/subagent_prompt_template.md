...
已满足 G1 硬触发后，再做五维评估，决定“不派 / 单子 Agent / 多子 Agent互证”：

| 维度 | 通过信号 | 风险信号 |
|---|---|---|
| 可并行性 | 子任务可独立完成 | 后续步骤依赖前序结果 |
| 通信需求 | 只需向主模型回报 | 成员需要互相协商接口或结论 |
| 上下文隔离收益 | 聚焦域不同，隔离可减少污染 | 所有任务共享同一背景 |
| 文件冲突风险 | 每个写者有互斥文件所有权 | 多个写者会改同一文件或同一生成物 |
| 成本收益 | 并行节省明显大于协调成本 | 主模型直接做更快 |

分流：

- 0-1 项通过：不派，主模型直接做。
- 2-3 项通过：优先单子 Agent，主模型集成。
- 4-5 项通过且文件所有权互斥：可多子 Agent 并行或互证。

并行写入前必须列出文件所有权清单；没有互斥清单，不并行。
...
| 2 | **范围**（白名单 + 黑名单）| 要改的文件/包路径清单；明确**不要改**的范围；并行时列文件所有权 | Agent 创建额外文件、动无关代码或与其他写者冲突 |
