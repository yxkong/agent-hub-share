...
| **Full Path** | 完整 Spec，落 `docs/plan/<domain>/` 过程稿或升格 `docs/design/` |
...
| **Required Outputs** | Task Contract（见 `templates/TEMPLATE_TASK_CONTRACT.md`） |
...
| **Required Outputs** | Release 检查 + Rollback 资产 |
| **Blockers** | 无回滚路径；online SQL 由 Agent 直接写入 |
| **Fast Path** | 开发环境 / 个人分支 → 轻量化 |
| **Full Path** | [release_gate.md](release_gate.md) + [rollback_gate.md](rollback_gate.md) |
...
| **Required Outputs** | insight / 反模式 / hub prompt / governance checklist 回填 |
| **Blockers** | 只写「下次注意」→ 门未过 |
| **Fast Path** | 一次性环境问题 + 明确原因 → 对话说明即可 |
| **Full Path** | 三路分流 + 若暴露治理缺口回填 [governance_checklist.md](governance_checklist.md) 或 [scorecard.md](scorecard.md) |
