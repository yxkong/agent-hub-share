...
> 核心原则：**git 内资产靠 git 回退；未纳入版本控制或远端资产先备份再修改；先确认再删除，除非明确要求不能删除。**
...
已纳入 git 的文档、脚本、规则和模板以 git 历史作为回退基线，不强制额外 `backup-file`。

以下场景仍必须备份：

- 未纳入版本控制的本地资产
- 远端配置文件
- 数据库、索引、PVC、日志等不可由 git 恢复的状态
- 用户明确要求额外备份

### 2.1 配置文件修改

**规则**：修改远端配置文件或未纳入版本控制的配置文件前，必须先备份原文件。
