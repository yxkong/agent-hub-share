...
- [ ] 存在 `## 修订记录` 章节，且按日期与版本**倒序**排列，最新记录位于表格首行
- [ ] 已在修订表**首行新增**一行（非仅改旧行，禁止追加到末尾）
...
- [ ] 若属于 material change（改变 trigger、门禁、目录、流程、公共行为），根 `README.md` §修订记录已在表格首行新增一行；纯错字 / 格式修正可在汇报中说明无需新增
...
- [ ] plan 最新修订记录（表格首行）写清「并入终版路径 + 终版版本」
- [ ] 终版修订表首行新增「整合 xxx plan」一行
