...
排序是硬约束：按日期与版本倒序，最新记录始终位于表格首行；同日按语义版本或修订序号倒序。新增记录插入表头分隔线后的第一行，禁止追加到表格末尾。
...
5. 修订记录表首行新增一行（要点 + 可选 ARCHIVE 路径）
...
过程稿（`docs/plan/`）在 `status: done` 时，修订表**最新一行（表格首行）**必须写清：

- 并入哪份终版路径；
- 终版版本号（如 `AI_RUNTIME_OBSERVABILITY_DESIGN.md v1.1`）。

终版整合后在修订表**首行新增**修订行，不要覆盖初稿历史，也不要追加到表格末尾。
