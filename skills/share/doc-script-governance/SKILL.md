...
备份粒度是“文件在本任务中的一次语义修改批次”：首次写入前备份一次即可；同一任务对同一文件的连续 Review/修正不得每轮重复备份。任务边界、目标版本或回退基线变化后再次修改，才创建新备份。备份是回退资产，不得反向成为最新文档的设计真源。
...
| `agent-asset-router` | 仅 engineering 项目中，工程产物在 skill / prompt / insight / docs / review 间未定时 |
