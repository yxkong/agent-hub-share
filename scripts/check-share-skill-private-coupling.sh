...
"
  ...
  if [ -d "$skill_root/references" ]; then
    files="$files$(find "$skill_root/references" -type f -name '*.md' ! -path "$skill_root/references/bak/*" ! -path "$skill_root/references/bak" 2>/dev/null || true)
"
  fi
  ...
  while IFS= read -r f || [ -n "$f" ]; do
    ...
    if grep -Eiq 'platform-jdk17|platform-admin|platform-review|platform-backend-dev|platform-core' "$f" 2>/dev/null; then
      first_match=$(grep -Eio 'platform-jdk17|platform-admin|platform-review|platform-backend-dev|platform-core' "$f" 2>/dev/null | head -n 1)
      record_violation "$f" "share skill 不得硬编码 private 项目或仓库名；请改为通用占位符或示例 first_match=${first_match}"
    fi
    if grep -Eiq 'admin-system-base(-frontend)?' "$f" 2>/dev/null; then
      first_match=$(grep -Eio 'admin-system-base(-frontend)?' "$f" 2>/dev/null | head -n 1)
      record_violation "$f" "share skill 不得硬编码 private 模块名；请改为 <frontend-app> / <domain-module> 等占位符 first_match=${first_match}"
    fi
    if grep -Eiq 'PLATFORM_DOCS_GOVERNANCE' "$f" 2>/dev/null; then
      record_violation "$f" '请将 PLATFORM_DOCS_GOVERNANCE 改为 docs/guide/DOCS_GOVERNANCE.md 或其他通用项目内命名'
    fi
    if grep -Eiq 'D:\\ai\\|D:/ai/' "$f" 2>/dev/null; then
      first_match=$(grep -Eio 'D:\\ai\\|D:/ai/' "$f" 2>/dev/null | head -n 1)
      record_violation "$f" "share skill 不得包含本机绝对路径；请改为 <hub-root> 或仓库相对路径 first_match=${first_match}"
    fi
    if grep -Eiq 'ai-hub-private|yxkong/ai-hub-private' "$f" 2>/dev/null; then
